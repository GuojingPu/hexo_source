...
&#8195;&#8195;
##### 1.不定长变量
不定长变量一般使用在参数不固定的情况下，常使用\*kargs表示一系列单一值（比如元组、列表），使用**kwargs表示一系列键值对（比如字典）
...
{'row': 1, 'colum': 2}
```
或者像这样（其实更多的情况下我们会这样用）：

```
my_list = [1,2,3]
my_dict = {'row':1,'colume':2}

fun_1(my_list,my_dict)
```
\*kargs和**kwargs会自动把列表和字典进行拆分开来,进行传入参数。
##### 2.闭包函数
